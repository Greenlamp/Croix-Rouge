 ...
 */
package GUI.Panels.Consultation;
...
public class M_Consultation extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        Gtableau.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Nom", "Prénom", "Date de Naissance", "Ville", "Profil complet"
            }
        ) {
    ...
    private LinkedList<String[]> trier(LinkedList<String[]> listeVolontaires) {
        LinkedList<String[]> listeTrier = new LinkedList<>();
        LinkedList<String[]> listeComplet = new LinkedList<>();
        LinkedList<String[]> listeIncomplet = new LinkedList<>();
        for(String[] elm : listeVolontaires){
            if(elm[4].equals("Complet")){
                listeComplet.add(elm);
            }else{
                listeIncomplet.add(elm);
            }
        }

        String nom = null;
        String[] buffer = null;
        int size = listeIncomplet.size();
        for(int i=0; i<size; i++){
            for(String[] elm : listeIncomplet){
                int cmp = 0;
                if(nom == null){
                    cmp = -1;
                }else{
                    cmp = elm[0].compareTo(nom);
                }
                if(cmp < 0){
                    nom = elm[0];
                    buffer = elm;
            ...
            listeTrier.add(buffer);
            listeIncomplet.remove(buffer);
            buffer = null;
            nom = null;
        }
        ...
        nom = null;
        buffer = null;
        size = listeComplet.size();
        for(int i=0; i<size; i++){
            for(String[] elm : listeComplet){
                int cmp = 0;
                if(nom == null){
                    cmp = -1;
                }else{
